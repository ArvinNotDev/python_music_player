...
class MusicPlayerUI(QWidget):
    def __init__(self):
        ...
        self.themes = {
            "dark": {"bg": QColor(18, 18, 18), "fg": QColor(230, 230, 230), "highlight": QColor(187, 134, 252), "button_bg": QColor(31, 31, 31), "button_fg": QColor(230, 230, 230)},
            "bright": {"bg": QColor(240, 240, 240), "fg": QColor(30, 30, 30), "highlight": QColor(98, 0, 238), "button_bg": QColor(224, 224, 224), "button_fg": QColor(30, 30, 30)}
        }
    ...
    def paintEvent(self, event):
        ...
        self.draw_list(painter, 30, 310, self.width() - 60, 310)
    ...
    def mouseMoveEvent(self, event):
        ...
        for name, rect in {**self.top_buttons,**self.bottom_buttons}.items():
            if rect.contains(pos): hovered=name; break
        if hovered!=self.hovered_button:
            self.hovered_button=hovered; self.update()

    def handle_top_button(self, name):
        if name=="playlists": self.current_view, self.selected_index = "playlists",0
        elif name=="favorites": self.current_view, self.selected_index = "favorites",0
        elif name=="theme": self.toggle_theme()
        elif name=="list": self.current_view, self.selected_index = "songs",0
        self.update()
    ...
    def handle_bottom_button(self, name):
        ...
        elif name=="volume_down": self.backend.volume_down()
        self.update()
    ...
    def keyPressEvent(self, event):
        ...
        if event.key()==Qt.Key.Key_Up and self.selected_index>0: self.selected_index-=1
        elif event.key()==Qt.Key.Key_Down and self.selected_index<max_idx: self.selected_index+=1
        vis = 310//30
        if self.selected_index < self.scroll_offset: self.scroll_offset=self.selected_index
        elif self.selected_index>=self.scroll_offset+vis: self.scroll_offset=self.selected_index-vis+1
        self.update()
